...
function FeatureContent({ title, items }: { title: string; items: FeatureItem[] }) {
  return (
    <>
      <h3 className="text-xl font-medium text-[#4E3E3B] mb-3">{title}</h3>
      <div className="space-y-2">
        {items.map((item, idx) => (
          <div key={idx}>
            <h4 className="text-sm font-medium text-[#4E3E3B]">{item.title}</h4>
            <p className="text-sm text-[#8A7F7C] italic">{item.description}</p>
          </div>
        ))}
      </div>
    </>
...
function FeatureCard({ title, items, media, index }: Feature & { index: number }) {
  ...
  return (
    <motion.div
      className={`w-full max-w-[800px] ${isEven ? 'ml-0 mr-auto' : 'ml-auto mr-0'}`}
      initial={{ opacity: 0, x: isEven ? -50 : 50 }}
      whileInView={{ opacity: 1, x: 0 }}
      viewport={{ once: true }}
      transition={{ duration: 0.6 }}
    >
      <div className="bg-[#F8F5F1] rounded-lg p-4 md:p-6 relative shadow-md overflow-hidden">
        <div className="flex flex-col md:flex-row items-center gap-4 md:gap-6">
          <div className={`hidden md:block md:w-2/5 ${isEven ? 'md:order-first' : 'md:order-last'}`}>
...
export default function Features() {
  return (
    <section className="py-8 md:py-16 relative overflow-hidden bg-[#F1EEE6]">
      <div className="container mx-auto px-4 md:px-6">
        <motion.div 
          ...
          transition={{ duration: 0.6 }}
        >
         <h2 className="text-2xl md:text-3xl font-normal mb-8 md:mb-16 text-[#B39793] transition-colors text-center">
