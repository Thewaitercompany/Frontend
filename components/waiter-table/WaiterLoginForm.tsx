...
export default function WaiterLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // const response = await fetch("https://backend-axu7.onrender.com/waiter/login", {
      //   method: "POST",
      //   headers: {
      ...
      const mockWaiterId = "W001";

      // Store auth data
      Cookies.set("waiterAuth", "true", { expires: 1 });
      Cookies.set(
        "waiterData",
        JSON.stringify({
      ...
      <div className="bg-[#ffffff] px-4 py-4">
        <div className="flex items-center">
          ...
          <span className="text-gray-900 text-base ml-2 flex items-center italic gap-1">
            <span className="text-xl">×</span> Staff Portal
          </span>
        </div>
      </div>
        ...
        <div className="w-full max-w-md bg-[#B39793] rounded-lg p-6 space-y-6">
          <div className="text-center">
            ...
            <p className="text-white/80 text-sm">
              Log in to access your restaurant dashboard
            </p>
          </div>
