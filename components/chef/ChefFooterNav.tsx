...
interface ChefFooterNavProps {
  restaurantId: string;
}

const ChefFooterNav: React.FC<ChefFooterNavProps> = ({ restaurantId }) => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 bg-white shadow-md">
      <div className="flex items-center justify-around py-2">
        <Link href={`/chef/${restaurantId}/dashboard`}>
          <div
        ...
        <Link href={`/chef/${restaurantId}/pending-orders`}>
          <div
            className={`flex flex-col items-center gap-1 p-2 ${
              isActive("/pending-orders") ? "text-[#B39793]" : "text-gray-500"
            }`}
          >
