...
function CardBoundary({ title, onViewMore, children }: CardBoundaryProps) {
  return (
    <div
      style={{
        borderRadius: 18,
...
function DashboardPage() {
  ...
  useEffect(() => {
    if (activeDescIndex !== null) {
      const close = (e: MouseEvent | globalThis.MouseEvent) => {
        const target = e.target as HTMLElement;
        if (
          !target.closest('.desc-popup') &&
          !target.closest('.view-more-btn')
        ) {
          setActiveDescIndex(null);
        }
      };
  ...
  function getDescPopupStyle(col: number, row: number): CSSProperties {
    return {
      position: "absolute",
      zIndex: 100,
      background: "#fff6f0",
