    ...
    <div className="bg-white rounded-lg shadow p-6">
      <div className="flex flex-wrap gap-2 mb-4 items-center">
        <div className="flex-1 flex items-center gap-2">
          ...
          <Button
            className="ml-2 bg-[#b39793] text-white rounded"
            style={{ minWidth: 90 }}
          >
            Search
          </Button>
      ...
      <div className="bg-[#fcfaf7] rounded-xl shadow p-6 mb-8">
        <div className="flex items-center mb-2">
          <Button variant="ghost" className="mr-2 px-2 py-1 text-[#b39793]">
            ←
          </Button>
          <div className="flex-1">
            <Input
              placeholder="Search Ingredient"
              className="pl-10 w-64 border border-[#b39793] bg-[#f5f1eb]"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>
        ...
        <div className="text-lg font-medium mb-2 mt-4">Potatoes</div>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead>Unit Used</TableHead>
              <TableHead>Category</TableHead>
              <TableHead>Spent</TableHead>
              <TableHead>Date/Time</TableHead>
              <TableHead>Unit Left</TableHead>
      ...
      <div>
        <h2 className="text-lg font-semibold mb-4">Items in Inventory</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
          {inventoryCards.map((item, i) => (
            <div
              key={i}
              className="bg-[#faf7f2] rounded-lg p-4 flex flex-col items-center shadow border border-[#e5e0d8] relative"
            >
              <img
                src={item.image}
                alt={item.name}
              ...
              />
              <div className="absolute top-2 right-2 bg-[#e5e0d8] text-xs rounded-full px-2 py-0.5">
                {item.qty}
              </div>
              <div className="font-medium text-base mb-1">{item.name}</div>
              <div className="text-xs text-[#b39793] mb-1">
                Category:{" "}
                <span className="text-[#4e3e3b]">{item.category}</span>
              </div>
              <div className="text-xs text-[#b39793] mb-2">
                Consumption:{" "}
                <span className="text-[#4e3e3b]">{item.consumption}</span>
              </div>
        ...
        <div className="flex items-center gap-2 mt-4">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            <span className="text-sm">Auto-Stock Items</span>
          </label>
          <div className="ml-auto text-xs text-gray-500">12 of 200 items</div>
        </div>
      </div>
    </div>
  );
